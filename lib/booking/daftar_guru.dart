...
class ListPageState extends  State<ListPage>{
  static List list_guru = [];
  Future<List<Guru>> getGuru() async {

    var data = await http.get(Uri.parse('https://tk-pbp-a01.herokuapp.com/booking/get_guru/'));

    var jsonData = json.decode(data.body);

    List<Guru> daftar_guru = [];

    for(var i in jsonData){
      Guru guru = Guru(i["nama_guru"], i["mata_pelajaran"], i["jenis_kelamin"]);
      daftar_guru.add(guru);
      list_guru.add(i["nama_guru"]);
    }
    return daftar_guru;
  }
          ...
          child: ListView(
                ...
                if (!snapshot.hasData) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index){
                      return cardBuild(snapshot.data[index].nama, snapshot.data[index].kelamin, snapshot.data[index].mapel);
                    },
                    );
                }
              } 
            ...
            padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 20),
              ...
              FloatingActionButton.extended( 
                ...
                onPressed: () {
                  Navigator.push(context, MaterialPageRoute(builder: (context) => MyBookingPage(),));
                },  
